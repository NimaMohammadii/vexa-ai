"""Runway Gen-4 image-to-video service."""
...
class VideoGen4Error(RuntimeError):
    """Raised when an image-to-video request fails."""


class VideoGen4Service:
    """Client wrapper around the Runway Gen-4 API."""

    _DEFAULT_BASE_URLS = (
        "https://api.dev.runwayml.com/v1",
        "https://api.runwayml.com/v1",
    )
    _MODEL = os.getenv("RUNWAY_GEN4_MODEL", "gen4_turbo")
    _API_VERSION = os.getenv("RUNWAY_API_VERSION", "2024-11-06")
    _DEFAULT_RATIO = os.getenv("RUNWAY_GEN4_RATIO", "16:9")
    _DEFAULT_DURATION = int(os.getenv("RUNWAY_GEN4_DURATION", "5"))
    _DEFAULT_FORMAT = os.getenv("RUNWAY_GEN4_FORMAT", "mp4")
    _REQUEST_TIMEOUT = 30
    ...
    def __init__(self) -> None:
        ...
        if not token:
            raise VideoGen4Error("کلید دسترسی Runway تنظیم نشده است.")
        ...
        self._session.headers.update(
            {
                ...
                "User-Agent": "vexa-ai-video-gen4/1.0",
    ...
    def generate_video(
        ...
        *,
        ...
        """Submit a Gen-4 image-to-video task and return the task ID."""

        if not image_bytes:
            raise VideoGen4Error("فایل تصویر معتبر نیست.")
        ...
        logger.debug("Submitting Gen-4 request", extra={"payload": log_payload})
        response = self._request("POST", "/image_to_video", json=payload)
        data = self._safe_json(response)

        task_id = self._extract_task_id(data) or data.get("id")
        if not task_id:
            logger.error("Missing task id in Gen-4 response", extra={"response": data})
            raise VideoGen4Error("شناسهٔ تسک از پاسخ Runway دریافت نشد.")
    ...
    def get_video_status(
        ...
        *,
        ...
        if not task_id:
            raise VideoGen4Error("شناسهٔ تسک معتبر نیست.")
        ...
        while time.time() < deadline:
            ...
            logger.debug("Gen-4 task %s status: %s", task_id, status)
            ...
            if status == "SUCCEEDED":
                ...
                if not video_url:
                    raise VideoGen4Error("خروجی ویدیو در پاسخ موفق پیدا نشد.")

                return {"url": video_url, "cover": cover_url}
            ...
            if status in {"FAILED", "CANCELED"}:
                ...
                raise VideoGen4Error(f"خطا: {error_msg}")

            time.sleep(poll_delay)

        raise VideoGen4Error("مهلت دریافت ویدیو به پایان رسید.")
        ...
        *,
        ...
        for index, base_url in enumerate(list(self._base_urls)):
            ...
            try:
                ...
                last_exception = exc
                logger.warning("Runway request failed", extra={"url": url, "error": str(exc)})
                continue

            if response.status_code == 404 and index + 1 < len(self._base_urls):
                logger.info(
                    "Runway endpoint not found, trying fallback",
        ...
        if last_exception is not None:
            raise VideoGen4Error(f"اتصال به Runway ناموفق بود: {last_exception}") from last_exception

        raise VideoGen4Error(
    ...
    def _fetch_assets(self, task_id: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            logger.debug("No assets available for Gen-4 task", extra={"task_id": task_id})
            return None
    ...
    def _safe_json(
        ...
        try:
            ...
            if default is not None:
                return default
            raise VideoGen4Error("پاسخ غیرقابل‌پارس از Runway دریافت شد.") from None
    ...
    def _extract_cover_url(cls, payload: Any) -> str | None:
        ...
        if isinstance(payload, str):
            text = payload.strip()
            if any(text.startswith(prefix) for prefix in ("http://", "https://", "//", "data:image")):
                return text
