...
if isinstance(data, dict):
    # بررسی کلیدهای معمول برای URL تصویر
    priority_keys = [
        'url', 'image_url', 'output_url', 'result_url',
        'download_url', 'file_url', 'asset_url', 'src',
        'href', 'link', 'path', 'uri', 'output'
    ]
    
    # ابتدا کلیدهای اولویت‌دار را بررسی کن
    ...
    # سپس سایر کلیدها را بررسی کن
    for key, value in data.items():
        if key.lower() not in [k.lower() for k in priority_keys]:
...
try:
    ...
    if not image_url:
        # اگر URL پیدا نشد، ساختار کامل پاسخ را لاگ کن
        logger.error(f"No image URL found. Response structure: {str(result)[:500]}...")
